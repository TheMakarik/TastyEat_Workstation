...
public sealed class ClientService(DataContext context, ILogger<ClientService> logger) : IClientService
{
    ...
    {
        var city = await context.Cities.FindAsync(new object[] { dto.CityId }, cancellationToken)
                   ?? throw new InvalidOperationException($"City with id {dto.CityId} not found.");

        Client? referrer = null;
        if (dto.ReferrerId.HasValue)
        {
            referrer = await context.Clients.FindAsync(new object[] { dto.ReferrerId.Value }, cancellationToken)
    ...
    public async Task<IReadOnlyList<ClientPurchaseShareDto>> GetPurchaseSharesAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            var total = await context.DistributionClients
                .AsNoTracking()
